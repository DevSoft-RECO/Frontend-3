<template>
  <div class="space-y-6">
    <!-- ELEMENTOS DE LA INTERFAZ WEB (print:hidden) -->
    ...
    <!-- VISTA EXCLUSIVA PARA IMPRESIÓN EN TABLAS LIMPIAS (print:block) -->
    <div class="hidden print:block space-y-6 text-black">
      <!-- Membrete -->
      <div class="border-b-2 border-black pb-4 mb-4 flex justify-between items-center">
        <div>
          <h1 class="text-xl font-bold uppercase tracking-wide">COOPERATIVA YAMAN KUTX, R.L.</h1>
          <h2 class="text-base font-semibold">REPORTE DE BALANCE DE INVENTARIO - LA CARTILLA GANADORA</h2>
          <p class="text-xs text-gray-700 mt-1">
            <strong>Ubicación / Agencia:</strong> {{ inventarioStore.balance?.agencia?.nombre || 'Central' }}
            ({{ inventarioStore.balance?.agencia?.codigo || 'GLOBAL' }})
          </p>
        </div>
        <div class="text-right text-xs text-gray-700">
          <p><strong>Fecha Emisión:</strong> {{ new Date().toLocaleDateString('es-GT') }}</p>
          <p v-if="filtros.fecha_inicio || filtros.fecha_fin">
            <strong>Rango:</strong> {{ filtros.fecha_inicio || 'Inicio' }} al {{ filtros.fecha_fin || 'Fin' }}
          </p>
        </div>
      </div>

      <!-- Tabla Resumen de Recursos (Cartillas y Stickers) -->
      <div>
        <h3 class="text-sm font-bold uppercase mb-2 border-b border-gray-400 pb-1">1. Resumen General de Recursos</h3>
        <table class="w-full text-left text-xs border border-gray-400 border-collapse">
          <thead>
            <tr class="bg-gray-200 text-black font-bold uppercase border-b border-gray-400">
              <th class="p-2 border-r border-gray-400">Recurso</th>
              <th class="p-2 text-center border-r border-gray-400">Entregados / Consumidos</th>
              <th class="p-2 text-center border-r border-gray-400">Stock Disponible</th>
              <th class="p-2 text-center">Total General</th>
            </tr>
          </thead>
          <tbody>
            <tr class="border-b border-gray-300">
              <td class="p-2 font-bold border-r border-gray-300">📋 Cartillas Físicas</td>
              <td class="p-2 text-center border-r border-gray-300">{{ inventarioStore.balance?.balance?.cartillas?.entregado || 0 }}</td>
              <td class="p-2 text-center border-r border-gray-300">{{ inventarioStore.balance?.balance?.cartillas?.disponible || 0 }}</td>
              <td class="p-2 text-center font-bold">{{ inventarioStore.balance?.balance?.cartillas?.total || 0 }}</td>
            </tr>
            <tr class="border-b border-gray-300">
              <td class="p-2 font-bold border-r border-gray-300">⭐ Stickers</td>
              <td class="p-2 text-center border-r border-gray-300">{{ inventarioStore.balance?.balance?.stickers?.entregado || 0 }}</td>
              <td class="p-2 text-center border-r border-gray-300">{{ inventarioStore.balance?.balance?.stickers?.disponible || 0 }}</td>
              <td class="p-2 text-center font-bold">{{ inventarioStore.balance?.balance?.stickers?.total || 0 }}</td>
            </tr>
          </tbody>
        </table>
      </div>

      <!-- Tabla Desglose de Promocionales -->
      <div>
        <h3 class="text-sm font-bold uppercase mb-2 border-b border-gray-400 pb-1">2. Desglose por Artículos Promocionales</h3>
        <table class="w-full text-left text-xs border border-gray-400 border-collapse">
          <thead>
            <tr class="bg-gray-200 text-black font-bold uppercase border-b border-gray-400">
              <th class="p-2 border-r border-gray-400">Artículo Promocional</th>
              <th class="p-2 text-center border-r border-gray-400">Estado</th>
              <th class="p-2 text-center border-r border-gray-400">Entregados</th>
              <th class="p-2 text-center border-r border-gray-400">Stock Disponible</th>
              <th class="p-2 text-center">Total (Entregado + Stock)</th>
            </tr>
          </thead>
          <tbody>
            <tr
            ...
            >
              <td class="p-2 font-bold border-r border-gray-300">{{ promo.nombre }}</td>
              <td class="p-2 text-center border-r border-gray-300">{{ promo.activo ? 'Activo' : 'Inactivo' }}</td>
              <td class="p-2 text-center border-r border-gray-300">{{ promo.entregado }}</td>
              <td class="p-2 text-center border-r border-gray-300">{{ promo.disponible }}</td>
              <td class="p-2 text-center font-bold">{{ promo.total }}</td>
            </tr>
            <tr v-if="!(inventarioStore.balance?.balance?.promocionales?.length)">
              <td colspan="5" class="p-4 text-center text-gray-600 italic">No hay promocionales registrados en el catálogo.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="pt-12 grid grid-cols-2 gap-8 text-center text-xs">
        <div>
          ...
          <p class="font-bold">Firma Encargado de Agencia / Almacén</p>
        </div>
        <div>
          <div class="border-t border-black w-48 mx-auto mb-1"></div>
          <p class="font-bold">Firma Verificación Mercadeo</p>
        </div>
      </div>
    </div>
  </div>
